...
export function handleVariableTokenBurn(event: VTokenBurn): void {
  ...
  let userReserve = getOrInitUserReserve(
    from,
    changetype<Address>(vToken.underlyingAssetAddress),
    event,
  )
  ...
  userReserve.currentTotalDebt = userReserve.currentStableDebt.plus(userReserve.currentVariableDebt)
...
export function handleStableTokenMint(event: STokenMint): void {
  ...
  poolReserve.lifetimePrincipalStableDebt =
    poolReserve.lifetimePrincipalStableDebt.plus(calculatedAmount)
